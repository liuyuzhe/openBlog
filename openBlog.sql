...
CREATE TABLE `attach_table` (
  ...
  `attach_slug` varchar(20) NOT NULL COMMENT '链接地址',
...
CREATE TABLE `post_table` (
  ...
  `post_status` enum('publish','draft','private') NOT NULL DEFAULT 'publish' COMMENT '文章状态',
  `post_comment_status` enum('open','close') NOT NULL DEFAULT 'open' COMMENT '评论状态',
  `post_type` enum('post','page') NOT NULL DEFAULT 'post' COMMENT '文章类型',
...
CREATE TABLE `term_table` (
  ...
  `term_slug` varchar(20) NOT NULL COMMENT '链接地址',
...
CREATE TABLE `user_table` (
  ...
  `user_avatars_url` varchar(100) DEFAULT NULL COMMENT '头像URL',
