...
public class RegisterFormValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        ValidationUtils.rejectIfEmptyOrWhitespace(errors, "nickname", null, "昵称不能为空");
        ...
        if (registerFormVo.getPassword() != registerFormVo.getRepeatPassword()) {
            errors.rejectValue("repeatPassword", null, "两次输入的密码不匹配");
        }
        if (!registerFormVo.getUsername().matches(RegularUtil.NICK_NAME)) {
            errors.rejectValue("nickname", null, "昵称输入非法");
        }
